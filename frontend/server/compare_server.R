  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$compare_btn, {
      ...
      if (input$input_mode == "csv") {
        # ── CSV upload mode ──
        if (is.null(input$file_a) || is.null(input$file_b)) {
          showNotification("Please upload both CSV files before comparing.", type = "warning")
          return()
        }
        ...
        path_a   <- input$file_a$datapath
        path_b   <- input$file_b$datapath
        source_a <- input$file_a$name
        source_b <- input$file_b$name
