...
echo "[MODULE lr_meta] starting" >> "${LOG_PATH}"

mkdir -p "${RUN_DIR}/results/report" "${RUN_DIR}/results/tables" "${RUN_DIR}/results/plots"

echo "This is a placeholder report for lr_meta." > "${RUN_DIR}/results/report/index.html"
...
echo "[MODULE lr_meta] wrote outputs.json + placeholders" >> "${LOG_PATH}"
exit 0
