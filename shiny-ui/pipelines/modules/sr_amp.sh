...
echo "[MODULE sr_amp] starting" >> "${LOG_PATH}"

mkdir -p "${RUN_DIR}/results/report" "${RUN_DIR}/results/tables" "${RUN_DIR}/results/plots"

echo "This is a placeholder report for sr_amp." > "${RUN_DIR}/results/report/index.html"
...
echo "[MODULE sr_amp] wrote outputs.json + placeholders" >> "${LOG_PATH}"
exit 0
